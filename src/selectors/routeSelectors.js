...
export const globalRoutesInfoSelector = createSelector(
  ...
  (routes, missionStatusTypesMap) => (routes.reduce((accumulator, route) => {
    // Count
    if (route.extraInfo.advancing > 99.9)
      accumulator.finishedRoutesCount++;
    accumulator.missionsCount += route.extraInfo.mission.count;
    accumulator.finishedMissionCount += Object.entries(route.extraInfo.mission.statusCounter).reduce((accumulator, [missionStatusTypeId, value]) => {
      if (missionStatusTypesMap[missionStatusTypeId].is_last)
        accumulator += value;
      return accumulator;
    }, 0);
    ...
    // Delay
    if (route.extraInfo.delay > 10)
    {
      accumulator.routeDelay++;
      if (route.extraInfo.delay > 30)
        accumulator.missionDelayOver30++;
    }
    return accumulator;
  }, {
